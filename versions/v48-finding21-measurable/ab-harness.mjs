...
const CLIENT_PERSONA =
  "You are role-playing a CLIENT being onboarded onto Lumen, a social listening tool. You are Dana, marketing lead at Northwind Athletics, a consumer-goods (athletic footwear & apparel) brand; your contact email is dana@northwind.example. In your FIRST couple of replies, state these specifics clearly and naturally, then do not repeat them unless you are asked: your three competitors are Velocity Sportswear, Apex Running Co, and Terra Gear; your current campaign is called Project Skylark; and your priority target markets are Japan, Brazil, and Norway. Your goals: protect brand reputation, track those three competitors, and catch customer issues early. Invent any other details consistently. Answer the assistant's latest message the way this client would: natural, cooperative, 1-2 sentences. If it shows options or a widget, just answer in plain language. When the assistant clearly signals the setup is complete and asks for final confirmation, reply exactly 'Yes, looks good.' Output ONLY the client's next message, no quotes, no commentary.";
...
async function runConversation(cfg) {
  ...
  } catch { /* leave recall null on a probe failure */ }
...
async function main() {
  ...
  for (const k of Object.keys(results)) {
    ...
    console.log(
      ...
      (((r.avgRecall ?? 0) * 100).toFixed(0) + "%").padEnd(9) +
  ...
  // Finding-21 verdict: the decision this harness exists to settle.
  const f = results["full-history"];
  if (base && f) {
    const costPct = base.avgCost ? (f.avgCost - base.avgCost) / base.avgCost * 100 : 0;
    const qDelta = (f.avgScore ?? 0) - (base.avgScore ?? 0);
    const rDelta = ((f.avgRecall ?? 0) - (base.avgRecall ?? 0)) * 100;
    console.log("\n--- Finding 21: full-history vs baseline (slide-" + MAX_HIST + ") ---");
    console.log("  cost " + (costPct >= 0 ? "+" : "") + costPct.toFixed(0) + "%   quality " + (qDelta >= 0 ? "+" : "") + qDelta.toFixed(2) + "/5   fact recall " + (rDelta >= 0 ? "+" : "") + rDelta.toFixed(0) + " pts");
    const qualityHolds = qDelta >= -0.2 && (f.avgRecall ?? 0) >= (base.avgRecall ?? 0) - 0.02;
    const notPricier = costPct <= 5;
    if (qualityHolds && notPricier)
      console.log("  READ: quality and recall hold (or improve) and it is not more expensive -> safe to raise/remove MAX_HIST_TURNS. Re-run at higher AB_RUNS to firm up before shipping.");
    else if (!qualityHolds)
      console.log("  READ: quality or recall dropped beyond noise -> do NOT adopt; keep the sliding window. (Investigate: did more context change behaviour?)");
    else
      console.log("  READ: quality holds but it costs more here -> weigh the trade-off; low AB_RUNS is noisy, re-run higher before deciding.");
  }
  console.log("\nRule of thumb: adopt a lever only if cost drops (or holds) AND quality/recall stays within noise of baseline. Bump AB_RUNS for tighter numbers.");
}
