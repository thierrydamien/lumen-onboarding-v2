...
const DOCX_MAX_XML = 12 * 1024 * 1024; // cap decompressed size — guards against a zip bomb (tiny file inflating huge)
async function inflateRawBounded(bytes, maxBytes) {
  ...
  for (;;) {
    ...
    if (done) break;
    total += value.length;
    if (total > maxBytes) { reader.cancel(); throw new Error("docx: decompressed content too large"); }
    chunks.push(value);
  }
...
async function docxToText(buf) {
  ...
  if (target.uncompSize && target.uncompSize > DOCX_MAX_XML) throw new Error("docx: document too large");
  ...
  let xmlBytes;
  if (target.method === 0) xmlBytes = comp;                                  // stored
  else if (target.method === 8) xmlBytes = await inflateRawBounded(comp, DOCX_MAX_XML); // deflate
