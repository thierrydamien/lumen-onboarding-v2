...
export default async (req) => {
  ...
  const { xlsxBase64, filename, clientEmail, company, contactName, topicsCount, usersCount } = body || {};
  ...
  if (appsUrl) {
    try {
      const r = await fetch(appsUrl, {
        ...
        body: JSON.stringify({ secret: process.env.APPS_SCRIPT_SECRET || "", xlsxBase64, filename: name, clientEmail: clientEmail || "", company: company || "", contactName: contactName || "", topicsCount, usersCount }),
      });
